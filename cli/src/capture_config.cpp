...
std::string resolve_config_path(const std::string& filename) {
    ...
    const fs::path candidate = fs::path(OPEN_RAW_RADAR_CONFIG_ROOT) / filename;
    if (fs::exists(candidate)) {
        return candidate.string();
    }
    throw std::runtime_error("Unable to resolve config path: " + filename);
}

std::string resolve_subnet(const std::string& subnet_override) {
    if (!subnet_override.empty()) {
...
std::string resolve_address_suffix(const std::string& suffix_override,
                                   const char* env_name,
...
NetworkConfig build_network_config(const std::string& subnet_override,
                                   const std::string& host_suffix_override,
                                   const std::string& board_suffix_override) {
    NetworkConfig network;
    network.subnet = resolve_subnet(subnet_override);
    network.host_suffix = resolve_address_suffix(host_suffix_override, "RADAR_HOST_SUFFIX", kDefaultHostSuffix);
    network.board_suffix = resolve_address_suffix(board_suffix_override, "RADAR_BOARD_SUFFIX", kDefaultBoardSuffix);
...
std::size_t compute_packets_per_frame(const std::size_t bytes_per_frame) {
...
CaptureConfig load_capture_config(const std::string& radar_config_filename,
    ...
    config.dca_config_path = resolve_config_path(dca_config_filename);
    ...
    config.network = build_network_config(subnet_override, host_suffix_override, board_suffix_override);
